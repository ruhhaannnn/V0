            ...
            if(textEl && dateEl) {
                ...
                if (extractMedia) {
                    ...
                    if (photoWrap && photoWrap.style.backgroundImage) {
                        ...
                        if (urlMatch && urlMatch[1]) mediaHTML = `<img src="${urlMatch[1]}" style="width:100%; border-radius:4px; border: 1px solid var(--border-color);" />`;
                    }
                    const videoWrap = msg.querySelector('video');
                    if (videoWrap && videoWrap.src) {
                        mediaHTML = `<video src="${videoWrap.src}" controls playsinline style="width:100%; max-height:400px; border-radius:4px; background: #000; border: 1px solid var(--border-color);"></video>`;
                    }
                }
...
async function fetchSummary() {
    try {
        ...
        let html = '';
        recent.slice(0, 10).forEach(p => { html += `<li><a href="${p.link}" target="_blank">${p.text}</a></li>`; });
...
async function syncToGlobalDatabase(newData) {
    console.log("Connect a backend database (Firebase/Supabase) to share data globally.");
}

function saveStoredIntel(dataArray) { 
    const slicedData = dataArray.slice(-2500);
    localStorage.setItem('iqwr_intel_db', JSON.stringify(slicedData)); 
    syncToGlobalDatabase(slicedData);
}
    ...
    try {
        ...
        posts.forEach(post => {
            ...
            if (evtType) {
                ...
                if (detectedLoc) {
                    // MAP REAL-TIME FIX: Lowered deduplication block from 2 hours to 15 minutes.
                    // This allows repeated attacks on the same city to register as fresh dots!
                    const isDuplicate = storedIntel.some(existing => 
                        existing.location === detectedLoc && existing.eventType === evtType && Math.abs(existing.timestamp - post.date.getTime()) < (15 * 60000)
                    );
...
function flyToLoc(lng, lat, strikeId) {
    ...
    if (markerInstances[strikeId]) {
        const data = markerInstances[strikeId];
        
        if (activePopup && activePopup !== data.popup) {
            activePopup.remove(); 
        }
        
        if (!data.popup.isOpen()) {
            data.popup.setLngLat([lng, lat]).addTo(map);
        }
        activePopup = data.popup;
    }
}
...
function renderMapData() {
    ...
    const filtered = globalIntelData.filter(d => {
        ...
        return true;
    }).sort((a,b) => a.timeAgo - b.timeAgo).slice(0, 150); 
    ...
    filtered.forEach(strike => {
        ...
        let hasMedia = (strike.mediaHTML && strike.mediaHTML.trim() !== '') ? true : false;
        ...
        let formattedMedia = '';
        if (hasMedia) {
            let cleanMedia = strike.mediaHTML.replace('controls', '');
            cleanMedia = cleanMedia.replace('<video', `<video autoplay loop muted playsinline onclick="this.muted=!this.muted; this.nextElementSibling.innerText = this.muted ? '🔇 TAP TO UNMUTE' : '🔊 MUTE'" style="cursor:pointer;" `);
            
            formattedMedia = `
                <div class="popup-media-container">
                    ${cleanMedia}
                    <div style="position:absolute; bottom:12px; right:8px; background:rgba(0,0,0,0.8); color:#fff; font-size:0.55rem; font-weight:bold; padding:4px 8px; border-radius:4px; pointer-events:none; border:1px solid rgba(255,255,255,0.3); z-index: 10;">
                        🔇 TAP TO UNMUTE
                    </div>
                </div>`;
        }
        ...
        const popupHTML = `
            ...
            <div style="font-size:0.7em; color:#888; text-transform:uppercase; border-top: 1px dashed #333; padding-top: 4px; margin-bottom: ${hasMedia ? '8px' : '0'};">SOURCE: ${strike.source}</div>
            ${formattedMedia}
        `; 
        ...
        const popupOptions = {
            offset: 10,
            closeOnClick: false,
            className: hasMedia ? 'popup-media-wide' : '' 
        };
        const popup = new maplibregl.Popup(popupOptions).setHTML(popupHTML);
        ...
        elContainer.addEventListener('click', (e) => { 
            e.stopPropagation(); 
            flyToLoc(strike.lng, strike.lat, strike.id); 
        });
        
        activeMapMarkers.push(marker);
        markerInstances[strike.id] = { marker: marker, popup: popup }; 
    });
}

// MAP REAL-TIME FIX: Decreased timer from 600s (10m) to 120s (2m)
let secondsLeft = 120; 
...
window.onload = () => {
    ...
    if(window.location.hash) {
        ...
        let navEl = document.querySelector(`[onclick*="${tabId}"]`);
        if(navEl) { switchTab(tabId, navEl); }
    }
};
...
map.on('load', () => {
    ...
    // MAP REAL-TIME FIX: Redraws the map markers every 60s so the "T-MINUS XX M" text physically updates
    setInterval(renderMapData, 60000); 
